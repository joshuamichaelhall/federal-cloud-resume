...
# Generate random suffix for S3 bucket name if not provided
resource "random_id" "bucket_suffix" {
  byte_length = 4
}

locals {
  bucket_name = var.s3_bucket_name != "" ? var.s3_bucket_name : "${var.project_name}-website-${random_id.bucket_suffix.hex}"
}
...
# Initialize the visitor counter with zero
